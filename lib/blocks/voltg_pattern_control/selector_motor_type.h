 ...
 * const VoltgChannelslNZ& pwmChannels = motorSelector.getPwmChannels();
 * @endcode
 */
...
private:
    VoltgChannelslNZ ChABCD;  ///< Array to store voltages for four channels
...
public:
     ...
     */
    SelectorMotorType(const MotorType& mode, const VectorAxes2D_I16& voltg_norm, const int16_t& sup_norm, const int16_t& brake)
        : mode_(mode), voltg_sup_(sup_norm), voltg_(voltg_norm), voltg_brak_(brake) {}
     ...
     */
    const VoltgChannelslNZ& getPwmChannels() const { return ChABCD; }

    // DO NOT USE SIMULTANEOUSLY WITH tick() !!!!!!!!!!!!!!!!!!!!
    void dev(VoltgChannelslNZ voltages) {
        ChABCD[ChannelA] = voltages[0];
        ChABCD[ChannelB] = voltages[1];
        ChABCD[ChannelC] = voltages[2];
        ChABCD[ChannelD] = voltages[3];
    }
};

void SelectorMotorType::math_coil(const int16_t& voltg_ref_, int16_t& voltg_out_1, int16_t& voltg_out_2) {
